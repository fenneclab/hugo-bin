...
import { fetchChecksums } from './checksums.js';
...
/**
 ...
 @param {Map<string, string>} [checksums] File name to sha256 digest. When given, the source(s) matching this OS/arch must have a matching entry.
 ...
 */
function createBin(baseUrl, hugoVersion, checksums, { extended = false, withDeploy = false } = {}) {
  ...
  for (const { os, arch, suffix, fallback } of targets) {
    ...
    bin = bin.src(`${baseUrl}${fileName}`, os, arch, digest && `sha256:${digest}`);
  }

  if (checksums) {
    // only the source(s) matching this OS/arch are ever downloaded, so that's
    // all that needs a checksum -- other platforms' entries may be missing
    const resolvedUrls = new Set(bin.resolvedUrls());
    const unverified = bin.src().filter(source => resolvedUrls.has(source.url) && !source.hash);
    if (unverified.length > 0) {
      throw new Error(`No checksum found for ${unverified.map(source => source.url).join(', ')}`);
    }
  }
...
/**
 ...
 @param {boolean} [options.verify] Fetch Hugo's published checksums and verify downloads against them.
 @returns {Promise<BinWrapper>} Wrapper that downloads and exposes the Hugo binary.
 */
export default async function main(cwd, { verify = false } = {}) {
  ...
  const checksums = verify ? await fetchChecksums(baseUrl, hugoVersion) : undefined;
  return createBin(baseUrl, hugoVersion, checksums, { extended, withDeploy });
}
