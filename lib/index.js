...
const extendedBin = new BinWrapper()
  .src(`${baseUrl}hugo_${hugoVersion}_extended_Linux-64bit.tar.gz`, 'linux', 'x64')
  .src(`${baseUrl}hugo_${hugoVersion}_extended_macOS-64bit.tar.gz`, 'darwin', 'x64')
  .src(`${baseUrl}hugo_${hugoVersion}_extended_Windows-64bit.zip`, 'win32', 'x64')
...
const normalBin = new BinWrapper()
  ...
  .dest(dest)
  .use(binName);
module.exports = extended ? extendedBin : normalBin;
